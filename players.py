...
class SARSAAgent:
    ...
    def train(self, num_episodes):
        n_visits = np.zeros((self.env.height, self.env.width), dtype=int)
        t = 0
        for episode in range(num_episodes):
            ...
            while not done:
                ...
                td_target = reward + self.gamma * self.q_values[next_state[0], next_state[1], next_action]
                td_error = td_target - self.q_values[state[0], state[1], action]
                eta_k = _eta_for(self.eta, state, t, n_visits)
                self.q_values[state[0], state[1], action] += eta_k * td_error
                
                n_visits[state[0], state[1]] += 1
                t += 1
    ...
    def train(self, num_episodes):
        ...
        for episode in range(num_episodes):
            ...
            while not done:
                ...
                # calc delta
                td_target = reward + self.gamma * self.q_values[next_state[0], next_state[1], action]
                td_error = td_target - self.q_values[state[0], state[1], action]
                eta_k = _eta_for(self.eta, state, t, n_visits)
                # update Q
                self.q_values[state[0], state[1], action] += eta_k * td_error
...
class TDLambdaAgent:
    def __init__(self, env, eta=0.5, gamma=0.9, epsilon=0.1, lam=0.9):
        ...
        self.rewards_per_episode = []
        self.value = np.zeros((self.env.height, self.env.width))
        self.policy = np.zeros((self.env.height, self.env.width), dtype=int)
    ...
    # IMPLEMENT ALGORITHM IN THE TRAIN FUNCTION #
    #############################################
    def tdpolicy_eval(self, num_episodes):
        N_visits = np.zeros((self.env.height, self.env.width))
        for episode in range(num_episodes):
            k = 0
            ...
            while not done:
                ...
                next, rew, done = self.env.step(action)
                V_prev = self.v_values.copy()
                # update z
                Z *= self.lam*self.gamma
                Z[x] += 1
                # calculate delta
                delta = rew + self.gamma * self.v_values[next]-self.v_values[x]
                # update v
                self.v_values = V_prev + self.eta*Z*delta
                # if value hasn't changed much
                if np.max(np.abs(self.v_values - V_prev)) <= self.epsilon:
                    break
                else:
                    k += 1
                    x = next
    ...
    def policy_improve(self):
        ...
        # next state is the best value of expected
        for x in range(self.env.height):
            for y in range(self.env.width):
                state = (x,y)
                if state in self.env.cliff or state == self.env.goal_state:
                    continue
    ...
    def train(self, num_episodes):
        # generate candidate policy - in initializer
        # i=0
        iterations = 0
        # while true:
        while True:
            old_v = self.v_values.copy()
            # policy evaluation - this is where TDlambda is applied
            self.tdpolicy_eval(num_episodes)
            # if v doesn't change, break
            if np.max(np.abs(self.v_values-old_v)) < self.epsilon:
                break
            # else
            else:
                # policy improvement
